#ifndef TESTLIB_TIMER
#define TESTLIB_TIMER
#include <chrono>

template <typename T>
std::string duration_to_string(T const &ns) {
...
#define timer_dur(ID)                                                          \
    std::chrono::duration_cast<std::chrono::nanoseconds>(_timer_end_##ID       \
                                                        - _timer_start_##ID)
#define timer_report(ID)                                                       \
    std::cout << "timer " #ID ": " << duration_to_string(timer_dur(ID)) << std::endl
