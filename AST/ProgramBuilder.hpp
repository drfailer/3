...
class ProgramBuilder {
      public:
        ...
        void
        beginBlock(); // create an empty block on the top of the blocks stack
        std::shared_ptr<Block>
        endBlock(); // pop the last block of the blocks stack
        void pushBlock(
            std::shared_ptr<ASTNode>); // add command to the last block

        std::shared_ptr<Funcall> createFuncall();

        std::shared_ptr<If> createIf(std::shared_ptr<ASTNode>,
                                     std::shared_ptr<Block>);
        std::shared_ptr<For> createFor(Variable, std::shared_ptr<ASTNode>,
                                       std::shared_ptr<ASTNode>,
                                       std::shared_ptr<ASTNode>,
                                       std::shared_ptr<Block>);
        std::shared_ptr<While> createWhile(std::shared_ptr<ASTNode>,
                                           std::shared_ptr<Block>);
      ...
      private:
        ...
        std::list<std::shared_ptr<Block>>
            blocks;                    // stack of blocks (the last
                                       // element is the current block)
        std::list<Variable> funParams; // parameters of the last function
        std::list<std::list<std::shared_ptr<TypedElement>>>
            funcallParams; // parameters of the last funcall
